...
from models.adl_recognition.schemas import ADLConfig
...
def classify_adl(person: dict, history: deque[dict], config: ADLConfig | None = None) -> tuple[str, float | None, str]:
    ...
    horizontal_torso = torso_angle >= config.torso_horizontal_low
    torso_lean = config.torso_lean_low <= torso_angle <= config.torso_lean_high
    ...
    if history and history[0].get("ankle"):
        ...
        frame_height = float(person.get("frame_height", height))
        velocity = math.hypot(ankle[0] - previous_ankle[0], ankle[1] - previous_ankle[1]) / max(frame_height, 1.0)
