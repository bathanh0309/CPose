...
from src.logging_utils import print_header, print_metric_table, print_saved
from src.manifest import ResolvedVideoItem, resolve_videos_from_manifest
from src.paths import ensure_dir, resolve_path
from src.timer import Timer
from src.topology import CameraTopology, load_camera_topology
...
def process_clip(
    ...
    clip_gids: set[int] = set()
    last_frame_gids: set[int] = set()
    last_frame_time: datetime | None = item.start_time
    frame_id = 0
    ...
    try:
        while True:
            ...
            current_frame_gids: set[int] = set()
            for person_idx, person in enumerate(persons):
                if not bool(person.get("is_confirmed", True)):
                    continue
                bbox = person.get("bbox", [0, 0, 0, 0])
                track_id = int(person.get("track_id") if person.get("track_id") is not None else -(person_idx + 1))
                ...
                clip_gids.add(gp.gid)
                current_frame_gids.add(gp.gid)
                draw_bbox(frame, bbox, "", (0, 220, 255))
                draw_skeleton(frame, person.get("keypoints", []))
                combined_label = f"person | T{track_id}"
                if adl_label:
                    combined_label += f" | {adl_label}"
                draw_global_id(frame, bbox, gp.gid, combined_label, gp.state)
            ...
            last_frame_gids = current_frame_gids
            ...
            writer.write(frame)
            if preview:
                if show_frame_preview(f"CPose Combined | {video_stem}", frame, info.fps):
                    preview = False
            frame_id += 1
    finally:
        capture.release()
    ...
    global_table.mark_dormant_missing(last_frame_gids, last_frame_time, float(config.get("max_candidate_age_sec", 300)))
    ...
    metrics = {
        ...
        "unique_global_ids": len(clip_gids),
...
def run_global_reid(multicam_dir: str | Path, output_dir: str | Path, **kwargs) -> list[dict]:
    return process_folder(multicam_dir, output_dir, **kwargs)
