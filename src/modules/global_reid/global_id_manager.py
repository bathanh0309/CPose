...
from src.modules.global_reid.matching import DEFAULT_WEIGHTS, cosine_similarity, extract_body_hsv_feature, extract_height_ratio, histogram_similarity, pose_signature, weighted_fusion
from src.modules.global_reid.state_machine import ACTIVE, CLOTHING_CHANGE_SUSPECTED, DORMANT, next_missing_state
...
class GlobalPersonTable:
    def __init__(self) -> None:
        ...
        self._next_gid = 1
        self.counts = {"new_id_count": 0, "strong_match_count": 0, "soft_match_count": 0, "ambiguous_match_count": 0, "topology_conflict_count": 0, "multi_candidate_conflict_count": 0}
    ...
    def match_or_create(self, bbox: list[float], frame: Any, camera_id: str, current_time: datetime | None, track_id: int, adl_label: str | None, keypoints: list[dict] | None, face_event: dict | None, topology: CameraTopology, config: dict) -> tuple[GlobalPerson, dict]:
        candidates = [person for person in self._table.values() if person.state != "CLOSED"]
