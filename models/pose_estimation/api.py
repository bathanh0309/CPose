...
from src.logging_utils import print_header, print_metric_table, print_saved, print_video_progress
...
from src.visualization import draw_bbox, draw_skeleton
from models.pose_estimation.metrics import build_pose_metrics
from models.pose_estimation.pose_model import PoseModel, resolve_pose_model
from models.tracking.tracker import iou
...
def _assign_track_ids(
    ...
    for person in persons:
        ...
        if best_track and best_score >= threshold:
            enriched["track_id"] = int(best_track["track_id"])
            enriched["is_confirmed"] = bool(best_track.get("is_confirmed", True))
            enriched["pose_track_iou"] = best_score
            if enriched.get("failure_reason") != "LOW_KEYPOINT_VISIBILITY":
                enriched["failure_reason"] = best_track.get("failure_reason", "OK")
            used.add(int(best_track["track_id"]))
        else:
            enriched["track_id"] = None
            enriched["is_confirmed"] = False
            enriched["pose_track_iou"] = None
            enriched["failure_reason"] = "NO_MATCHED_TRACK"
...
def process_video(
    ...
    try:
        while True:
            ...
            persons = _assign_track_ids(persons, tracks, track_iou_threshold, run_on_confirmed_tracks_only)
            for person in persons:
            ...
            writer.write(frame)
            if preview:
                if show_frame_preview(f"CPose Pose | {video_path.stem}", frame, info.fps):
                    preview = False
            frame_id += 1
    finally:
        capture.release()
...
def run_pose_estimation(input_dir: str | Path, output_dir: str | Path, **kwargs) -> list[dict]:
    return process_folder(input_dir, output_dir, **kwargs)


__all__ = ["_assign_track_ids", "process_folder", "process_video", "run_pose_estimation"]
