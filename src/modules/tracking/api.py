"""Public API for CPose Module 2: local tracking."""
...
from src.modules.tracking.metrics import build_tracking_metrics
    ...
    try:
        while True:
            ...
            for track_item in tracks:
                ...
                draw_track(frame, track_item["bbox"], track_item["track_id"], track_item["confidence"], track_history=track_history[t_id])
            records.append({
                "frame_id": frame_id,
            ...
            writer.write(frame)
            if preview:
                if show_frame_preview(f"CPose Tracking | {video_path.stem}", frame, info.fps):
                    preview = False
            frame_id += 1
    finally:
        capture.release()
...
def process_folder(input_dir: str | Path, output_dir: str | Path, model: str | Path | None = None, tracker: str = "bytetrack.yaml", conf: float = 0.5, detection_dir: str | Path | None = None, min_hits: int = 3, max_age: int = 30, window_size: int = 30, preview: bool = True) -> list[dict]:
    videos = list_video_files(input_dir)
    output_dir = ensure_dir(output_dir)
    print_header("CPose Person Tracking Module")
    print_metric_table({"Input folder": resolve_path(input_dir), "Output folder": output_dir, "Model": resolve_tracking_model(model), "Tracker": tracker, "Videos found": len(videos)})
    results = []
    ...
    print_metric_table({"Total videos": len(results), "Total frames": sum(item.get("total_frames", 0) for item in results), "Total tracks": sum(item.get("total_tracks", 0) for item in results), "Average FPS": f"{sum(item.get('fps_processing', 0.0) for item in results) / len(results):.2f}" if results else "0.00"})
    return results
