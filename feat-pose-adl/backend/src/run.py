...
Pose + ADL + ReID with G-ID overlays only.
"""
import sys
import cv2
import csv
...
class SequentialRunner:
    ...
    def __init__(self, config_path=None, headless=False):
        ...
        # Default to shared pose/ADL config
        if config_path is None:
            config_path = REPO_ROOT / "configs" / "pose_adl.yaml"
        
        with open(config_path, 'r', encoding='utf-8') as f:
            self.config = yaml.safe_load(f)
        
        print("="*60)
        print("HAVEN Sequential: Pose + ADL + ReID")
        print("="*60)
        ...
        self.metrics_path = BACKEND_DIR.parent / "metrics_log.md"
        self.total_frames_processed = 0
        self.start_pipeline_time = time.time()
        self.total_det_latency_ms = 0
        self.det_count = 0
        self.adl_counts = {}
        self.danger_counts = {}
        self.id_loss_count = 0
        self.cam_stats = {}
        ...
        # Registration is removed. The pipeline only shows G-IDs + pose + ADL.
        print("Label mode: G-ID only")
        
        # YOLO Pose (Keypoints)
        yolo_cfg = self.config['yolo']
        pose_model_path = self._resolve_model_path(
            yolo_cfg.get('model_path'),
            default=REPO_ROOT / "models" / "yolo11n-pose.pt",
        ...
        print(f"Pose Model: {pose_model_path.name}")
        
        # YOLO Detect (Objects - Danger)
        det_model_path = self._resolve_model_path(
            yolo_cfg.get('danger_model_path'),
            default=REPO_ROOT / "models" / "yolo11n.pt",
            fallback=BACKEND_DIR / "models" / "yolo11n.pt",
        )
    ...
    @staticmethod
    def _resolve_model_path(configured_path, default, fallback):
        candidates = []
        if configured_path:
    ...
    def process_camera(self, cam_config, cam_index):
        ...
        if not video_path_obj.is_absolute():
            video_path_obj = REPO_ROOT / video_path_obj
        ...
        while not self._stop_flag:
            if not paused:
                ...
                self.cam_stats[cam_id]['frames_processed'] += 1
                ...
                for box in det_results[0].boxes:
                    ...
                    if cls_id in self.danger_classes:
                        ...
                        if label not in danger_objects:
                            self.danger_counts[label] = self.danger_counts.get(label, 0) + 1
                ...
                det_ms = (time.time() - start_det) * 1000
                self.total_det_latency_ms += det_ms
                self.det_count += 1
                ...
                if results[0].boxes and results[0].boxes.id is not None:
                    ...
                    if track_ids.size > 0:
                        self.cam_stats[cam_id]['frames_with_id'] += 1
                    ...
                    for i, track_id in enumerate(track_ids):
                        ...
                        if kpts is not None:
                            ...
                            if state.current_posture and state.current_posture != old_posture:
                                self.adl_counts[state.current_posture] = self.adl_counts.get(state.current_posture, 0) + 1
                ...
                if lost_tracks:
                    self.cam_stats[cam_id]['id_loss_events'].append((frame_idx, len(lost_tracks)))
                    for track_id in lost_tracks:
                        self.id_loss_count += 1
                        del track_states[track_id]
                
                # UI
                is_master_cam = (cam_id == self.config.get('master_camera', 'cam1'))
                draw_ui_panel(display, cam_id, frame_idx, total_frames, is_master_cam, global_ids_seen)
        ...
        cap.release()
        self.cam_stats[cam_id]['end_time'] = time.time()
    ...
    def cleanup(self):
        ...
        if hasattr(self, 'global_id_manager'):
            ...
            total_time = end_pipeline_time - self.start_pipeline_time
            avg_fps = self.total_frames_processed / total_time if total_time > 0 else 0
            avg_det_latency = self.total_det_latency_ms / self.det_count if self.det_count > 0 else 0
            
            try:
                with open(self.metrics_path, "a", encoding="utf-8") as f:
                    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    f.write(f"\n## Báo Cáo Chạy Hệ Thống lúc `{timestamp}`\n\n")
                    f.write(f"### 1. Hiệu Năng Tổng Quan (System Performance)\n\n")
                    f.write(f"| Thông số | Kết quả |\n")
                    f.write(f"|---|---|\n")
                    f.write(f"| **Tổng thời gian xử lý** | {total_time:.2f} giây |\n")
                    f.write(f"| **Tổng Frame đã xử lý** | {self.total_frames_processed} frame |\n")
                    f.write(f"| **FPS trung bình (Avg)** | {avg_fps:.2f} fps |\n")
                    f.write(f"| **Độ trễ Detect (YOLO)** | {avg_det_latency:.2f} ms/frame |\n\n")
                    
                    f.write(f"### 2. Thống Kê Theo Camera\n\n")
                    f.write(f"| Tên Camera | Avg FPS | Số frame (Processed) | Số frame (Có ID) | Sự kiện Rớt ID | Tại mốc Frame |\n")
                    f.write(f"|---|---|---|---|---|---|\n")
                    for cid, cstat in self.cam_stats.items():
                        c_time = cstat['end_time'] - cstat['start_time'] if cstat['end_time'] > 0 else (time.time() - cstat['start_time'])
                        c_fps = cstat['frames_processed'] / c_time if c_time > 0 else 0
                        frames_proc = cstat['frames_processed']
                        frames_id = cstat['frames_with_id']
                        loss_count = len(cstat['id_loss_events'])
                        
                        id_percent = (frames_id / frames_proc * 100) if frames_proc > 0 else 0
                        loss_frames_str = "_Không có_"
                        if cstat['id_loss_events']:
                            loss_frames = [str(ev[0]) for ev in cstat['id_loss_events'][:10]]
                            elipsis = "..." if len(cstat['id_loss_events']) > 10 else ""
                            loss_frames_str = f"[{', '.join(loss_frames)}{elipsis}]"
                            
                        f.write(f"| **{cid.upper()}** | {c_fps:.2f} fps | {frames_proc} | {frames_id} ({id_percent:.1f}%) | {loss_count} lần | {loss_frames_str} |\n")
                        
                    f.write(f"\n### 3. Tần Suất Hành Động & Vật Nguy Hiểm (ADL & Danger)\n\n")
                    f.write(f"| Nhóm Phân Loại | Đối tượng / Hành vi | Số lần/số khung ghi nhận |\n")
                    f.write(f"|---|---|---|\n")
                    has_adl_or_danger = False
                    for act, count in self.adl_counts.items():
                        f.write(f"| Hành vi (ADL) | {act} | {count} sự kiện |\n")
                        has_adl_or_danger = True
                    for obj, count in self.danger_counts.items():
                        f.write(f"| Vật nguy hiểm | {obj} | {count} khung hình |\n")
                        has_adl_or_danger = True
                    if not has_adl_or_danger:
                        f.write(f"| _(Trống)_ | _Không ghi nhận_ | 0 |\n")
                    f.write(f"\n---\n")
                print(f"Metrics Log saved: {self.metrics_path}")
            except Exception as e:
