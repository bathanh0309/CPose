"""Global ID table and TFCS-PAR identity manager."""
...
from src.topology import CameraTopology, infer_zone, topology_score
from models.global_reid.matching import DEFAULT_WEIGHTS, cosine_similarity, extract_body_hsv_feature, extract_height_ratio, histogram_similarity, pose_signature, weighted_fusion
from models.global_reid.state_machine import ACTIVE, CLOTHING_CHANGE_SUSPECTED, DORMANT, next_missing_state
...
class GlobalPersonTable:
    def __init__(self) -> None:
        ...
        self._next_gid = 1
        self._vote_buffer: dict[tuple[str, int], dict[int, int]] = {}
        self.counts = {"new_id_count": 0, "strong_match_count": 0, "soft_match_count": 0, "ambiguous_match_count": 0, "pending_vote_count": 0, "topology_conflict_count": 0, "multi_candidate_conflict_count": 0}
    ...
    def _candidate_scores(self, person: GlobalPerson, bbox: list[float], frame: Any, camera_id: str, current_time: datetime | None, keypoints: list[dict] | None, face_event: dict | None, topology: CameraTopology, config: dict) -> dict[str, Any]:
        ...
        total = weighted_fusion({"face": score_face, "body": score_body, "pose": score_pose, "height": score_height, "time": score_time, "topology": score_topology}, weights)
        return {"person": person, "score_total": total, "score_face": score_face, "score_body": score_body, "score_pose": score_pose, "score_height": score_height, "score_time": score_time, "score_topology": score_topology, "topology_allowed": bool(topo.get("allowed")), "delta_time_sec": delta, "entry_zone": entry_zone, "exit_zone": person.last_exit_zone, "failure_reason": topo.get("failure_reason", "OK"), "body_feat": body_feat, "height_ratio": height_ratio, "pose_signature": pose_feat, "face_embedding": face_embedding}

    def _pre_filter_candidates(self, camera_id: str, current_time: datetime | None, config: dict) -> list[GlobalPerson]:
        max_age = float(config.get("max_candidate_age_sec", 300))
        top_k = int(config.get("top_k_candidates", 0) or 0)
        candidates: list[GlobalPerson] = []
        for person in self._table.values():
            if person.state == "CLOSED":
                continue
            if current_time and person.last_time:
                delta = (current_time - person.last_time).total_seconds()
                if delta > max_age:
                    continue
            candidates.append(person)
        candidates.sort(key=lambda person: person.last_time or datetime.min, reverse=True)
        return candidates[:top_k] if top_k > 0 else candidates

    def match_or_create(self, bbox: list[float], frame: Any, camera_id: str, current_time: datetime | None, track_id: int, adl_label: str | None, keypoints: list[dict] | None, face_event: dict | None, topology: CameraTopology, config: dict) -> tuple[GlobalPerson, dict]:
        candidates = self._pre_filter_candidates(camera_id, current_time, config)
        scored = [self._candidate_scores(person, bbox, frame, camera_id, current_time, keypoints, face_event, topology, config) for person in candidates]
        ...
        if not viable:
            ...
            self._vote_buffer.pop((camera_id, track_id), None)
            ...
            if second and abs(float(best["score_total"]) - float(second["score_total"])) < margin:
                ...
                person = best["person"]
                confirm_frames = max(1, int(config.get("confirm_frames", 1)))
                vote_key = (camera_id, track_id)
                votes = self._vote_buffer.setdefault(vote_key, {})
                votes[person.gid] = votes.get(person.gid, 0) + 1
                for gid in list(votes):
                    if gid != person.gid:
                        del votes[gid]
                if votes[person.gid] >= confirm_frames:
                    match_status = "strong_match"
                    self.counts["strong_match_count"] += 1
                    votes.clear()
                else:
                    match_status = "pending_vote"
                    self.counts["pending_vote_count"] += 1
        ...
        match_info = {key: best.get(key) for key in ("score_total", "score_face", "score_body", "score_pose", "score_height", "score_time", "score_topology", "topology_allowed", "delta_time_sec", "entry_zone", "exit_zone", "failure_reason")}
    ...
    def _fresh_scores(self, bbox: list[float], frame: Any, keypoints: list[dict] | None, face_event: dict | None, topology: CameraTopology, camera_id: str) -> dict[str, Any]:
        return {"score_total": None, "score_face": None, "score_body": None, "score_pose": None, "score_height": None, "score_time": None, "score_topology": None, "topology_allowed": None, "delta_time_sec": None, "entry_zone": infer_zone(camera_id, bbox, "entry", topology), "exit_zone": None, "failure_reason": "OK", "body_feat": extract_body_hsv_feature(frame, bbox), "height_ratio": extract_height_ratio(bbox, frame.shape[0] if frame is not None else None), "pose_signature": pose_signature(keypoints, bbox), "face_embedding": face_event.get("embedding") if face_event else None}
    ...
    def mark_dormant_missing(self, active_gids: set[int], current_time: datetime | None, max_candidate_age_sec: float = 300.0) -> None:
        ...
        for person in self._table.values():
            if person.gid in active_gids or person.last_time is None:
                continue
            person.state = next_missing_state(person.state, (current_time - person.last_time).total_seconds(), max_candidate_age_sec)

    def to_dict(self) -> dict[str, Any]:
        return {person.global_id: {"state": person.state if person.state != DORMANT else DORMANT, "last_camera_id": person.last_camera_id, "last_time": person.last_time.isoformat() if person.last_time else None, "last_adl": person.last_adl, "history": person.history} for person in self._table.values()}


class GlobalIDManager(GlobalPersonTable):
    pass


__all__ = ["GlobalIDManager", "GlobalPerson", "GlobalPersonTable"]
