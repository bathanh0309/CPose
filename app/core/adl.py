...
class ADLConfig:
    ...
    # Default values
    TORSO_ANGLE_LAYING = float(_ADL_CFG.get("torso_angle_laying", 35))
    ASPECT_RATIO_LAYING = float(_ADL_CFG.get("aspect_ratio_laying", 0.9))
    KNEE_ANGLE_SITTING = float(_ADL_CFG.get("knee_angle_sitting", 140))
    ...
    def from_dict(cls, adl_config: dict):
        ...
        if adl_config:
            cls.TORSO_ANGLE_LAYING = adl_config.get("torso_angle_laying", cls.TORSO_ANGLE_LAYING)
            cls.ASPECT_RATIO_LAYING = adl_config.get("aspect_ratio_laying", cls.ASPECT_RATIO_LAYING)
            cls.KNEE_ANGLE_SITTING = adl_config.get("knee_angle_sitting", cls.KNEE_ANGLE_SITTING)
            cls.MOVEMENT_THRESHOLD_RATIO = adl_config.get("movement_threshold_ratio", cls.MOVEMENT_THRESHOLD_RATIO)
            cls.MOVEMENT_WALKING_MULTIPLIER = adl_config.get(
                "movement_walking_multiplier",
                cls.MOVEMENT_WALKING_MULTIPLIER,
            )
