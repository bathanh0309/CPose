...
function initSocket() {
  ...
  socket.on("analysis_progress", (msg) => {
    ...
    if (clip) {
      ...
      clip.progress = msg.pct / 100;
      clip.labelsWritten = msg.labels_written;
      renderClipQueue();
    }
  });
  ...
  socket.on("analysis_complete", (msg) => {
    ...
    renderClipQueue();
    toast(`Tự động gán nhãn hoàn tất: ${msg.clips_done} clip`, "success");
  });
}
...
const refs = {
  ...
  batchStopBtn: $("batchStopBtn"),
  outputDirLabel: $("outputDirLabel"),
  changeOutputDirBtn: $("changeOutputDirBtn"),
  slots: [
...
function renderClipQueue() {
  ...
  refs.clipQueue.innerHTML = state.clips.map((clip) => {
    ...
    const isSelected = state.selectedClips.has(clip.id);

    return `
      <div class="queue-card ${clip.status} ${isSelected ? 'selected' : ''}" data-id="${clip.id}">
        <div style="display:flex; align-items:flex-start; gap:8px;">
          <input type="checkbox" class="clip-selector" data-id="${clip.id}" ${isSelected ? 'checked' : ''} style="margin-top:4px;" />
          <details style="flex:1;">
            ...
            <div class="queue-grid" style="margin-top: 6px;">
              ...
              <div><strong>Labels</strong><span>${escapeHtml(clip.labelsWritten || 0)}</span></div>
  ...
  refs.clipQueue.querySelectorAll(".clip-selector").forEach(cb => {
    cb.addEventListener("change", (e) => {
      const id = cb.dataset.id;
      if (e.target.checked) state.selectedClips.add(id);
...
function boot() {
  ...
  refs.selectAllClips.addEventListener("change", (e) => {
    if (e.target.checked) {
      state.clips.forEach(c => state.selectedClips.add(c.id));
    } else {
  ...
  refs.batchProcessBtn.addEventListener("click", async () => {
    if (state.selectedClips.size === 0) {
      toast("Vui lòng chọn ít nhất 1 video", "warning");
      return;
    }
    
    toast(`Bắt đầu gán nhãn cho ${state.selectedClips.size} video`, "info");
    try {
      // For Phase 2, we just start it on the folder where most clips are
      // because the backend current implementation scans the whole folder.
      // In a more advanced version, we'd pass the list of files.
      ...
      const data = await res.json();
      if (res.ok) {
        toast("Tiến trình gán nhãn đã khởi chạy", "success");
      } else {
        toast(data.error || "Lỗi khởi chạy", "error");
      }
    } catch (e) {
  ...
  refs.batchStopBtn.addEventListener("click", async () => {
    await fetch("/api/analysis/stop", { method: "POST" });
    toast("Đã yêu cầu dừng gán nhãn", "info");
  });

  refs.changeOutputDirBtn.addEventListener("click", () => {
    const newPath = prompt("Nhập đường dẫn lưu kết quả nhãn (Phase 2):", state.outputPath);
    if (newPath) {
      state.outputPath = newPath;
      refs.outputDirLabel.textContent = `Out: ${newPath}`;
      toast(`Cập nhật đường dẫn lưu: ${newPath}`, "info");
    }
  });
