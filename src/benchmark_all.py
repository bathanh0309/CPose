...
def benchmark(run_dir: str | Path, save_csv: bool = False) -> dict[str, Any]:
    run_path = Path(run_dir)
    reid_summary_path = run_path / "05_global_reid" / "reid_metrics.json"
    reid_metric_paths = [
        path
        for path in run_path.rglob("reid_metrics.json")
        if path.resolve() != reid_summary_path.resolve()
    ]
    metrics = {
        ...
        "reid": [_load_json(path, {}) for path in reid_metric_paths],
    }
    reid_summary = _load_json(reid_summary_path, {})
    ...
    summary: dict[str, Any] = {
        ...
        "global_id_count": evaluation.get("modules", {}).get("reid", {}).get("global_id_count") or reid_summary.get("total_global_ids"),
...
def main() -> None:
    ...
    parser.add_argument("--save-csv", action="store_true", help="Also write benchmark_summary.csv")
    args = parser.parse_args()
    setattr(args, "output", args.run_dir)
    print_module_console("pipeline", args)
    benchmark(args.run_dir, save_csv=args.save_csv)
