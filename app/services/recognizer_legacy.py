...
class PoseADLRecognizer:
    ...
    def _run(self, clips: list[Path], output_dir: Path, model_path: Path, save_overlay: bool) -> None:
        ...
        try:
            ...
            adl_cfg = self._config.get("adl_model_cfg", "research/common/configs/ctr_gcn_adl.yaml")
        ...
        for clip in clips:
            ...
            try:
                ...
                # Inline merger to get global IDs for this clip
                clip_global_map = {}
                if output_dir.exists():
                    try:
                        logger.info(f"Running cross-camera ID merger up to {clip.name}...")
                        merger = CrossCameraIDMerger(output_dir, processed_clips)
                        merger.merge()
                        # Extract the map for current clip
                        for (c_stem, local_id), global_id in merger.processed_ids_map.items():
                            if c_stem == clip.stem:
                                clip_global_map[local_id] = global_id
                    except Exception as merger_exc:
                        logger.warning(f"Merger failed: {merger_exc}")
    ...
    def _process_clip(self, model, adl_model, clip: Path, output_dir: Path, save_overlay: bool, socketio) -> tuple[int, int, dict]:
        ...
        local_tracks = {} # track_id -> _TrackState
        ...
        return keypoints_written, adl_events, local_tracks
    ...
    def _render_preview_video(self, clip: Path, output_dir: Path, local_tracks: dict, clip_global_map: dict, socketio):
        started_at = time.time()
        cap = cv2.VideoCapture(str(clip))
        if not cap.isOpened():
            return
        ...
        clip_output_dir = output_dir / clip.stem
        preview_path = clip_output_dir / f"{clip.stem}_preview.mp4"
        fourcc = cv2.VideoWriter_fourcc(*"mp4v")
        out_video = cv2.VideoWriter(str(preview_path), fourcc, fps, (width, height))
        
        kp_file = clip_output_dir / f"{clip.stem}_keypoints.txt"
        adl_file = clip_output_dir / f"{clip.stem}_adl.txt"
            ...
            with open(kp_file, "r") as f:
                for line in f:
                    ...
                    for i in range(0, len(nums), 3):
                        xy.append([nums[i], nums[i+1]])
                        conf.append(nums[i+2])
                    frame_data[frame_id].append({"gid": gid, "xy": np.array(xy), "conf": np.array(conf)})
                    
        adl_data = defaultdict(dict)
        if adl_file.exists():
            with open(adl_file, "r") as f:
                for line in f:
                    ...
                    label = parts[2]
                    conf = float(parts[3])
                    adl_data[frame_id][gid] = (label, conf)
                    
        global_to_local = {g: l for l, g in clip_global_map.items()}
        cam_id = extract_multicam_camera_id(clip) or "unknown"
        ...
        while True:
            ...
            current_fps = frame_id / max(0.1, elapsed)
            
            cv2.putText(frame, f"Cam: {cam_id.upper()} | Frame: {frame_id}/{total_frames} | FPS: {current_fps:.1f}", (15, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            
            persons = frame_data.get(frame_id, [])
            for p in persons:
                gid = p["gid"]
                xy = p["xy"]
                conf = p["conf"]
                
                valid_pts = xy[conf > min_conf]
                if len(valid_pts) > 0:
                    x_min, y_min = np.min(valid_pts, axis=0)
                    x_max, y_max = np.max(valid_pts, axis=0)
                    padding = 20
                    x_min, y_min = int(x_min - padding), int(y_min - padding)
                    x_max, y_max = int(x_max + padding), int(y_max + padding)
                    
                    cv2.rectangle(frame, (x_min, y_min), (x_max, y_max), (60, 140, 255), 2)
                    
                    local_id = global_to_local.get(gid, "?")
                    if gid == -1:
                        gid_str = "UNR"
                    else:
                        gid_str = str(gid)
                        
                    adl_label, adl_conf = adl_data.get(frame_id, {}).get(gid, ("pending", 0.0))
                    
                    label = f"L{local_id}|G{gid_str} {adl_label} {adl_conf:.2f}"
                    text_origin = (x_min, max(18, y_min - 8))
                    cv2.putText(frame, label, text_origin, cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 2, cv2.LINE_AA)
                    cv2.putText(frame, label, text_origin, cv2.FONT_HERSHEY_SIMPLEX, 0.55, (20, 20, 20), 1, cv2.LINE_AA)
                
                frame = draw_skeleton(frame, xy, conf, min_conf=min_conf)
                
            out_video.write(frame)
            
            if frame_id % PROGRESS_EVERY == 0:
                jpeg_proc = cv2.imencode('.jpg', frame)[1].tobytes()
                self._update_state(snapshot_processed=jpeg_proc)
                
            frame_id += 1
            
        out_video.release()
        cap.release()
        logger.info(f"Rendered preview video for {clip.name} to {preview_path.name}")
