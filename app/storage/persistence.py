...
_PERSISTENCE_DEFAULTS = get_runtime_section("persistence")
DEFAULT_EMBEDDING_DIM = int(_PERSISTENCE_DEFAULTS.get("embedding_dim", 512))
INITIAL_MEMMAP_SIZE = int(_PERSISTENCE_DEFAULTS.get("initial_memmap_size", 10000))
EXPAND_STEP = int(_PERSISTENCE_DEFAULTS.get("expand_step", 1000))
EMA_ALPHA = float(_PERSISTENCE_DEFAULTS.get("ema_alpha", 0.3))
...
class PersistenceManager:
    ...
    def __init__(self, persist_path: str, embedding_dim: int = DEFAULT_EMBEDDING_DIM):
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Person profiles table (name, age, gender)
            conn.execute("""
    ...
    def _create_new_memmap(self):
        ...
        # Create new memmap with configurable initial capacity
        initial_size = INITIAL_MEMMAP_SIZE
        self.embeddings = np.memmap(
            str(self.embeddings_path),
            dtype='float32',
            mode='w+',
    ...
    def register_global_id(
        ...
        with self.lock:
            ...
            if embedding_idx >= self.embeddings.shape[0]:
                self._expand_embeddings(embedding_idx + EXPAND_STEP)
    ...
    def register_profile(self, global_id: int, name: str, age: int = None, gender: str = None):
        """Register person profile for a GlobalID."""
        with self.lock:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute("""
                    ...
                    (global_id, name, age, gender, created_at)
                    VALUES (?, ?, ?, ?, ?)
                """, (global_id, name, age, gender, timestamp))
                conn.commit()
            logger.info(f"Registered profile for G{global_id}: {name}, {age}, {gender}")
    
    def get_profile(self, global_id: int) -> Optional[Dict]:
        """Get person profile for a GlobalID."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT name, age, gender FROM person_profiles WHERE global_id=?",
                (global_id,)
            )
            result = cursor.fetchone()
            if result:
                return {'name': result[0], 'age': result[1], 'gender': result[2]}
        return None
